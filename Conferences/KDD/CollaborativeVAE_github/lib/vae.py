    ...
    def run_all(self, data_x, lr, batch_size, epoch, print_step=100, x_valid=None):
        ...
        with tf.variable_scope("inference"):
            rec = {'W1': tf.get_variable("W1", initializer=tf.constant(self.weights[0]), dtype=tf.float32),
                   ...
                   'W_z_log_sigma': tf.get_variable("W_z_log_sigma", initializer=tf.constant(self.weights[3]),
                                                    dtype=tf.float32),
                   'b_z_log_sigma': tf.get_variable("b_z_log_sigma", initializer=tf.constant(self.biases[3]),
                                                    dtype=tf.float32)}
        ...
        for i in range(epoch):
            ...
            if (i + 1) % print_step == 0:
                if x_valid is None:
                    ...
                    logging.info(
                        'epoch {0}: batch loss = {1}, gen_loss={2}, latent_loss={3}, valid_loss={4}'.format(i, l, gl,
                                                                                                            ll,
                                                                                                            valid_loss))
    ...
    def run(self, data_x, hidden_dim, activation, loss, lr,
        ...
        if loss == 'rmse':
            ...
            loss = -tf.reduce_mean(tf.reduce_sum(
                x_ * tf.log(tf.maximum(decoded, 1e-16)) + (1 - x_) * tf.log(tf.maximum(1 - decoded, 1e-16)), 1))
        train_op = tf.train.AdamOptimizer(lr).minimize(loss)
