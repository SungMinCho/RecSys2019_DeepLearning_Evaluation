...
from keras.layers import Embedding, Input, Dense, merge, Reshape, Flatten, concatenate, RepeatVector, \
    multiply  # , Merge
...
def get_model(usize, isize, path_nums, timestamps, length, layers=[20, 10], reg_layers=[0, 0], latent_dim=40,
    ...
    umtm_latent = get_umtm_embedding(umtm_input, path_nums[0], timestamps[0], length, user_latent, item_latent,
                                     path_attention_layer_1, path_attention_layer_2)
    umum_latent = get_umum_embedding(umum_input, path_nums[1], timestamps[1], length, user_latent, item_latent,
                                     path_attention_layer_1, path_attention_layer_2)
    umtmum_latent = get_umtmum_embedding(umtmum_input, path_nums[2], timestamps[2], length, user_latent, item_latent,
                                         path_attention_layer_1, path_attention_layer_2)
    uuum_latent = get_uuum_embedding(uuum_input, path_nums[3], timestamps[3], length, user_latent, item_latent,
                                     path_attention_layer_1, path_attention_layer_2)
    ...
    model = Model(inputs=[user_input, item_input, umtm_input, umum_input, umtmum_input, uuum_input],
                  outputs=[prediction])

    return model


def get_train_instances(user_feature, item_feature, type_feature, path_umtm, path_umum, path_umtmum, path_uuum,
                        path_nums, timestamps, train_list, num_negatives, batch_size, shuffle=True):
...
if __name__ == '__main__':
    ...
    t1 = time()
    (ps, rs, ndcgs) = evaluate_model(model, user_feature, item_feature, type_feature, num_users, num_items, path_umtm,
                                     path_umum, path_umtmum, path_uuum, path_nums, timestamps, length, testRatings,
    ...
    for epoch in range(epochs):
        ...
        train_steps, train_batches = get_train_instances(user_feature, item_feature, type_feature, path_umtm, path_umum,
                                                         path_umtmum, path_uuum, path_nums, timestamps, train,
        ...
        if epoch % verbose == 0:
            (ps, rs, ndcgs) = evaluate_model(model, user_feature, item_feature, type_feature, num_users, num_items,
                                             path_umtm, path_umum, path_umtmum, path_uuum, path_nums, timestamps,
