    ...
    def walk_umum(self, s_u, e_m):
        ...
        for m in self.um_dict[s_u]:
            sim = self.get_sim(self.user_embedding[s_u],
                               self.item_embedding[m])  # self.user_embedding[s_u].dot(self.item_embedding[m]) / 
            m_list.append([m, sim])
        ...
        for u in self.mu_dict.get(e_m, []):
            sim = self.get_sim(self.item_embedding[e_m],
                               self.user_embedding[u])  # self.item_embedding[e_m].dot(self.user_embedding[u])
            u_list.append([u, sim])
