        ...
        for column_index in range(data_to_print.shape[1]):
            ...
            if np.isnan(result_to_print):
                ...
                if highlight_best and (
                        ...
                        (row_index == len(algorithm_data_to_print_list) - 1 and not column_better_than_threshold_found[
                            column_index])):
...
def print_parameters_latex_table(result_folder_path,
        ...
        for experiment_subfolder in experiment_subfolder_list:
            ...
            try:
                ...
                if parameters_list is None:
                    ...
                    assert parameters_list == list(
                        parameters_dict.keys()), "n_parameters {}, len(parameters_dict) {}".format(parameters_list,
                                                                                                   list(
    ...
    for row_index in range(len(algorithm_data_to_print_list)):
        ...
        experiment_subfolder_to_parameters_dict, parameters_list = get_parameter_values_for_algorithm(
            algorithm_file_name)
            ...
            for current_parameter in parameters_list:
                ...
                for experiment_subfolder in experiment_subfolder_list:
                    ...
                    if experiment_subfolder_to_parameters_dict[experiment_subfolder] is None:
                        ...
                        parameter_value = experiment_subfolder_to_parameters_dict[experiment_subfolder][
                            current_parameter]

                    if parameter_value is None:
                        algorithm_parameter_new_line += "- \t"

                    elif not isinstance(parameter_value, list) and any(substring in current_parameter for substring in
                                                                       ["penalty", "rate", "reg", "l1_ratio",
                                                                        "lambda"]):
        ...
        for experiment_subfolder in [result_folder_path]:
            ...
            if result_dict is not None:
                ...
                if len(result_dict["train_time_list"]) > 1:
                    result_row_string += "{:.{n_decimals}f} $\\pm$ {:.{n_decimals}f} [s]\t&".format(mean, stddev,
                    ...
                    if new_time_unit == "s":
                        ...
                        result_row_string += "{:.{n_decimals}f} [{}] / {:.{n_decimals}f} [{}]\t&".format(mean, "s",
                                                                                                         new_time_value,
                                                                                                         new_time_unit,
                                                                                                         n_decimals=n_decimals)
